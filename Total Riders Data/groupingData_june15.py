...
### Load Data
df = pd.read_csv("Data/WMATA_Ridership_Summary.xlsx - June_15_2024.csv")
df.columns = df.columns.str.strip()

### Evening Riders (7pm-12am)
...
data_june15 = {
    "Time Period": ["Evening (7pm-12am)", "Late Night (12am-Close)", 
                    "PM Peak (3pm-7pm)", "Midday (9:30am-3pm)", "AM Peak (Open-9:30am)"], 
    "Total Riders": [total_evening_riders, total_late_night_riders, 
                     total_pm_peak_riders, total_midday_riders, total_am_peak_riders]
}

index = ['0', '1', '2', '3', '4']
df_june15 = pd.DataFrame(data_june15, index=index)

df_june15.to_csv('june_15.csv', index=True, index_label="Indexes", header=True)

print("Total ridership data for June 15, 2024, has been saved in 'june_15.csv'.")
